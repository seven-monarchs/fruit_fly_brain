"""
tests/test_wall_spawn_south.py

Tests spawning further south (y=-2) with the proven wall config.

Rationale:
  - Proven config (spawn y=0, heading=0.588) hits wall at y~7, exits y~10 = 2mm detour
  - All attempts to widen the wall (y=0..10) or change heading (0.3, 0.4) fail:
    T5 oversteer prevents crossing x=14 when gap is further north
  - Root cause: approach angle and T5 dynamics change when gap y is higher
  - Solution: keep proven config (heading=0.588, wall y=0..9) but spawn at y=-2
    -> fly hits wall at y~5 instead of y~7
    -> needs 4mm northward travel (vs 2mm) to reach gap at y>=9
    -> same approach angle = same stable T5 dynamics = no BADQACC
  - Bonus: fly path is more dramatic (clearly curves north, then northeast to food)
...
Run:
    wenv310\Scripts\python.exe tests/test_wall_spawn_south.py
"""
...
SPAWN_POS        = (0, -2, 0.2)   # shifted 2mm south vs proven (0, 0, 0.2)
SPAWN_HEADING    = (0, 0, 0.588)  # proven stable heading
...
def run_scenario(use_wall, use_flyvis, network, rm, t5a_idx, t5b_idx,
    ...
    if use_wall:
        ...
        # Print full trajectory near x=14 for analysis
        near_wall = (x > 13.5) & (x < 14.5)
        if near_wall.any():
            steps_near = np.where(near_wall)[0]
            print(f"    y when x~14: {[round(y[i],2) for i in steps_near[:8]]}")
...
print("=" * 65)
print("Spawn y=-2  |  Heading=0.588 rad  |  Wall y=0..9, gap y>=9")
print("=" * 65)
print("Proven config (spawn y=0): hits wall at y~7, exits y~10 (2mm detour)")
print("This test (spawn y=-2):    hits wall at y~5, exits y~9  (4mm detour)")
print()
...
print("\n" + "=" * 65)
print("VERDICT  (spawn y=-2, heading=0.588, wall y=0..9, gap y>=9)")
print("=" * 65)
...
if r_a and r_b and r_c:
    print("\n  -> ALL PASS - update main sim: SPAWN_POS=(0,-2,0.2)")
elif r_a and r_b and not r_c:
    print("\n  -> Flyvis navigates, odor-only fails")
elif r_a and not r_b and r_c:
    print("\n  -> Odor-only works, flyvis still fails - T5 oversteer persists")
    print("     Try spawn y=-1 (intermediate) or reduce BIAS_MAX to 0.08")
elif r_a and not r_b and not r_c:
    print("\n  -> Both fail - 4mm detour is too large, gap y=9 unreachable from y=5")
    print("     Odor equilibrium may be pulling fly away from gap")
else:
    print("\n  -> Mixed result")

print("\nFor reference: proven config -> A=78, B=89, C=92 steps, crossing y~10.2-10.4")
